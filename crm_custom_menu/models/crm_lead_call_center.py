...
class CrmLead(models.Model):
    ...
    from_callcenter = fields.Boolean("From Call Center", default=False)
...
class CrmLeadCallCenter(models.Model):
    ...
    # New field to store the resolved supervisor when assigned_supervisor_id is empty
    ...
    # New fields to store existing salesperson info for duplicate numbers
    ...
    # New fields to store temer.lead information
    ...
    def _onchange_validate_phone(self):
        for record in self:
            if record.new_phone and record.country_id and record.country_id.code:
                phone_call_center = record.new_phone.strip()
                # Special case for Ethiopia (ET): allow 09... or 07... with 10 digits
                if record.country_id.code.upper() == 'ET':
                    if (
                    (phone_call_center.startswith('09') or phone_call_center.startswith('07'))
                    and len(phone_call_center) == 10
                    and phone_call_center.isdigit()
                ):
                        return  # Accept as valid, skip further validation
                try:
                    parsed = phonenumbers.parse(phone_call_center, record.country_id.code)
    ...
    def _extract_salesperson_from_message(self, message):
        """
        Extract salesperson name from phone_number_message
        Expected format: "In CRM Leads registered by: Salesperson Name."
        """
        if not message:
            return None
            
        # Pattern to match salesperson name from the message
        patterns = [
            r'registered by: ([^\.]+)\.',
            r'by: ([^\.]+)\.',
            r'registered by ([^\.]+)\.'
        ...
        for pattern in patterns:
            ...
            if match:
                salesperson_name = match.group(1).strip()
    ...
    def _get_existing_salesperson_info(self, full_phone_number):
        ...
        # If not found in temer.lead, check in crm.lead as fallback
        crm_lead = self.env['crm.lead'].search([
            ('phone_ids', '=', full_phone_number),
            ('is_expired', '!=', True)
        ], order='create_date desc', limit=1)
        
        if crm_lead and crm_lead.user_id:
            salesperson = crm_lead.user_id
            supervisor = self._get_supervisor_for_salesperson(salesperson)
    ...
    def create(self, vals):
        # Get the next available round-robin user and config
        ...
        if vals.get('new_phone'):
            ...
            # Check for duplicates and get message
            ...
            # Store phone number in callcenter phone table
    ...
    def _create_temer_lead_automatically(self):
        ...
        _logger.info("=== AUTOMATIC TEMER.LEAD CREATION ===")
        ...
        # Check if from_callcenter field exists before adding it
        try:
            if hasattr(self.env['temer.lead'], 'from_callcenter'):
                lead_values['from_callcenter'] = True
        except Exception:
            _logger.warning("from_callcenter field does not exist in temer.lead model")
        ...
        try:
            ...
            # Don't raise error to allow call center record to be saved
    ...
    def _send_sms_for_duplicate(self):
        ...
        _logger.info("=== AUTOMATIC SMS FOR DUPLICATE PHONE ===")
    ...
    def _get_duplicate_phone_message(self, full_phone_number, exclude_ids=None):
        ...
        messages = [msg for msg in [temer_lead_msg, crm_lead_msg, crm_reception_msg, crm_website_msg] if msg]
    ...
    def _send_sms(self, mobile_number, message):
        ...
        try:
            ...
            sender = 'Temer RE'  # Your registered sender name Temer RE
            from_identifier = 'e80ad9d8-adf3-463f-80f4-7c4b39f7f164'  # Your identifier ID from AfroMessage e80ad9d8-adf3-463f-80f4-7c4b39f7f164
    ...
    def _send_sms_to_sales_person(self, lead, sales_person, customer_phone, is_existing=False):
        ...
        try:
            ...
            if is_existing:
                message = f"Hi {sales_name}, Your customer with no. {customer_phone} came via {source}. Please contact and work to close."
            else:
    ...
    # Remove the manual action_create_crm_lead method since it's now automatic