...
class CrmWebsite(models.Model):
    ...
    # Supervisor nomination & assignment fields
    ...
    def _check_source_id(self):
        ...
        for record in self:
            if website_group and self.env.user in website_group.users:
                website_source = self.env['utm.source'].search([('name', '=', 'Website')], limit=1)
                if record.source_id != website_source:
    ...
    def _create_temer_lead_automatically(self):
        ...
        _logger.info("=== AUTOMATIC TEMER.LEAD CREATION FOR WEBSITE ===")
        ...
        lead_values = {
            ...
            'state': 'prospect',
            'from_website': True,  # Mark as from website
        }
        ...
        try:
            ...
            # Don't raise error to allow website record to be saved
    ...
    def _send_sms_for_duplicate(self):
        ...
        _logger.info("=== AUTOMATIC SMS FOR DUPLICATE PHONE (WEBSITE) ===")
