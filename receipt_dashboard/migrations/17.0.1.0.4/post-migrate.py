...
def migrate(cr, version):
    ...
    with registry(cr.dbname).cursor() as new_cr:
        ...
        for payment_line in payment_lines:
            ...
            try:
                # Use the payment line's creator as the environment user
                ...
                total_created += 1
                _logger.info(
                    'Backfill: created draft receipt record for payment line %s (ref: %s) on reservation %s (status: %s)',
                    payment_line.id, payment_line.ref_number or 'N/A', reservation.id, reservation.status,
                )
