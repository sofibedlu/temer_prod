...
class PropertyReservationHistory(models.Model):
    ...
    @api.constrains('reservation_type_id', 'property_id', 'status')
    def _check_quick_reservation_unique(self):
        """Ensure there's at most one active quick reservation per property."""
        for rec in self:
            if not rec.reservation_type_id or not rec.property_id:
                continue
            try:
                is_quick = rec.reservation_type_id.reservation_type == 'quick'
            except Exception:
                is_quick = False
            if is_quick:
                quick_conf_ids = self.env['property.reservation.configuration'].search([
                    ('reservation_type', '=', 'quick')
                ]).ids
                domain = [
                    ('id', '!=', rec.id),
                    ('property_id', '=', rec.property_id.id),
                    ('reservation_type_id', 'in', quick_conf_ids),
                    ('status', 'not in', ['canceled', 'expired', 'sold'])
                ]
                if self.search(domain, limit=1):
                    raise ValidationError(_('A Quick Reservation already exists for this property.'))
