...
class TeamAjwaDashboard extends Component {
    setup() {
            ...
            this.state.chart = new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Team Ajwa - Lead Status Distribution'
                        },
                    },
                    scales: {
                        y: {
            ...
            this.state.pieChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        title: {
                            display: true,
                            text: 'Team Ajwa - Lead Status Distribution (Pie)'
                        },
                        legend: {
                            position: 'right'
                        }
    ...
    async loadDashboardData() {
        ...
        try {
            ...
            const filteredLeads = leads.filter(lead => wingMap.get(lead.wing_id ? lead.wing_id[0] : 0) === 'Team - Ajwa');
            ...
            const leadEvents = filteredLeads.map(lead => {
                ...
                return {
                    ...
                    wing_name: lead.wing_id ? wingMap.get(lead.wing_id[0]) || 'Team - Ajwa' : 'Team - Ajwa',
                    wing_manager_name: '',
                    event_type
                };
            });
            ...
            reservations.forEach(res => {
                ...
                if (!lead) return;
                const key = `${lead.wing_id ? wingMap.get(lead.wing_id[0]) || 'Team - Ajwa' : 'Team - Ajwa'}||${lead.supervisor_id ? this.cleanSupervisorName(supervisorMap.get(lead.supervisor_id[0])) || 'No Supervisor' : 'No Supervisor'}|${lead.user_id ? userMap.get(lead.user_id[0]) || 'No Sales Person' : 'No Sales Person'}`;
            ...
            allKeys.forEach(key => {
                ...
                const record = {
                    wing_name: String(ec.wing_name || rc.wing_name || 'Team - Ajwa'),
    ...
    setSupervisorFilterFromLink(supervisorId) {
        ...
        const dashboardEl = document.querySelector('.o_team_ajwa_dashboard');
        if (dashboardEl) {
...
TeamAjwaDashboard.template = "team_dashboard.TeamAjwaDashboardTemplate";
registry.category("actions").add("TeamAjwaDashboardTemplate", TeamAjwaDashboard);