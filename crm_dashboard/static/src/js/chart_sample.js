...
//     }
    ...
    async fetchData() {
        try {
            ...
            // Fetch order lines with all filters including date range
            this.data = await this.orm.searchRead(
                "sale.order.line", 
                domain, 
                ["product_id", "order_id", "price_total", "create_date"]
            );
          
            // Pass date range to get_top_customers_chart if needed
            const context = {};
            if (this.dateRange.value) {
                const [startDate, endDate] = this.dateRange.value.split(" - ");
                context.date_from = startDate;
                context.date_to = endDate;
            }
            
            this.sales_data = await this.orm.call(
                "sale.order", 
                "get_top_customers_chart", 
                [],
                { context }
            );
            
            console.log('Fetched data with date range:', this.dateRange.value, this.data);
            this.renderChart();
        } catch (error) {
    ...
    async fetchStats() {
        // Build domain with date range if specified
        const domain = [];
        if (this.dateRange.value) {
            const [startDate, endDate] = this.dateRange.value.split(" - ");
            domain.push(['date_order', '>=', startDate]);
        ...
        const salesDomain = [['state', '=', 'sale'], ...domain];
        const quotationsDomain = [['state', 'in', ['draft', 'sent']], ...domain];
        const ordersDomain = [['state', 'in', ['sale', 'draft', 'sent']], ...domain];
        
        const [totalSales, totalQuotations, totalOrders] = await Promise.all([
            this.orm.searchRead("sale.order", salesDomain, ["amount_total"]),
            this.orm.searchRead("sale.order", quotationsDomain, ["amount_total"]),
            this.orm.searchRead("sale.order", ordersDomain, ["amount_total"])
        ]);
    
        const totalSalesAmount = totalSales.reduce((sum, order) => sum + (order.amount_total || 0), 0);
        const totalQuotationsAmount = totalQuotations.reduce((sum, order) => sum + (order.amount_total || 0), 0);
        const totalOrdersAmount = totalOrders.reduce((sum, order) => sum + (order.amount_total || 0), 0);
        
        // Calculate total revenue with proper number formatting
        const totalRevenueAmount = Math.max(1, totalSalesAmount); // Prevent division by zero
        ...
        this.stats.totalQuotations = totalQuotations.length;
        this.stats.totalOrders = totalOrders.length;
        this.stats.totalRevenue = formattedRevenue;
    
        // Calculate percentages
        this.stats.salesPercent = (totalSalesAmount / totalRevenueAmount) * 100;
        this.stats.quotationsPercent = totalQuotationsAmount > 0 ? (totalQuotationsAmount / totalRevenueAmount) * 100 : 0;
        this.stats.ordersPercent = (totalOrdersAmount / totalRevenueAmount) * 100;
        this.stats.revenuePercent = 100; // Total revenue is 100% of itself
    }
    ...
    onDateRangeChange(event) {
        this.dateRange.value = event.target.value;
        // Update both data and stats when date range changes
        Promise.all([
            this.fetchData(),
            this.fetchStats()
        ]).catch(error => {
