...
class ImportEmpEntryWizard(models.TransientModel):
    _name = "import.entry.custom.cl.wizard"
    _description = "Import Employee Entry Wizard"
    ...
    def import_employee_entry_apply(self):
        employee_entry_cl_obj = self.env['employee.entry.custom.checklist']
        if self:
            for rec in self:
                # For CSV
                if rec.import_type == 'csv':
                    ...
                    try:
                        ...
                        skip_header = True
                        created_entry_cl = False
                        created_entry_cl_list = []
                        for row in myreader:
                            try:
                                if skip_header:
                                    skip_header = False
                                ...
                                if row[0] not in (None, "") and row[1] not in (None, ""):
                                    vals = {}
                                    if row[0] != '':
                                        vals.update({'name': row[0]})
                                    if row[1] != '':
                                        vals.update(
                                            {'description': row[1]})

                                        created_entry_cl = employee_entry_cl_obj.create(
                                            vals)
                                        created_entry_cl_list.append(
                                            created_entry_cl.id)
                    ...
                    if counter > 1:
                        completed_records = len(created_entry_cl_list)
