...
class DownloadReport(http.Controller):
    ...
    @http.route(['/download/payslip/<int:payslip_id>'], type='http', auth="public", website=True)
    def download_payslip(self, payslip_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access(
                'hr.payslip', payslip_id, access_token=access_token)
        except (AccessError, MissingError):
            return '<br/><br/><center><h1><b>Oops Invalid URL! Please check URL and try again!</b></h1></center>'
        report_type = 'pdf'
        download = True
        return self._show_report(model=order_sudo, report_type=report_type, report_ref='sh_hr_payroll.action_report_payslip', download=download)
