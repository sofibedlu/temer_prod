...
class AttendanceWizard(models.TransientModel):
    ...
    def calculate_attendance(self):
        ...
        for log in attendance_log:
            if log.employee_id.id in employee_list:
                attd = self.check_in_check_out(log.employee_id.id)
                if attd:
                    attd.write({'check_out': log.punching_time})
                employee_list.remove(log.employee_id.id)
            else:
                attd = self.check_in_check_out(log.employee_id.id)
                if attd and not attd.check_out:
                    attd.write({'check_out': log.punching_time})
                else:
    ...
    def check_in_check_out(self, emp_id):
        attendances = self.env['hr.attendance'].search([('employee_id', '=', emp_id), ('check_out', '=', False)], limit=1)
        return attendances