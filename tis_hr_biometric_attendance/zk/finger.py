...
class Finger(object):
    ...
    def json_unpack(json):
        return Finger(
            ...
            fid=json['fid'],
            valid=json['valid'],
            template=codecs.decode(json['template'].encode('ascii'), 'hex')
        )
    ...
    def dump(self):
        return "<Finger> [uid:{:>3}, fid:{}, size:{:>4} v:{} t:{}]".format(self.uid, self.fid, self.size, self.valid, codecs.encode(self.template, 'hex').decode('ascii'))